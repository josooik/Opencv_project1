...
writer1 = cv2.VideoWriter('process.mp4', codec, fps, (width,height))
...
while True:
    ...
    cont = cv2.hconcat([img_frame, img_dst_y])
    cont1 = cv2.hconcat([img_dst_w, img_dst_yw])
    ...
    writer.write(img_dst_yw)

    img_frame2 = cv2.pyrDown(cont2)
    writer1.write(img_frame2)

    cv2.imshow('Video', img_frame2)
    cv2.imshow('img_dst_yw', img_dst_yw)
