# Opencv 차선인식(canny + HoughLineP 출력)
import numpy as np
import cv2
...
writer = cv2.VideoWriter('output.mp4', codec, fps, (2560,720))
writer1 = cv2.VideoWriter('process.mp4', codec, fps, (3840,2880), isColor=True)
...
while True:
    ...
    cont3 = cv2.hconcat([img_grays1, img_gausss, frame_binarys])
    cont4 = cv2.vconcat([cont, cont1, cont2, cont3])

    result_cont1 = cv2.hconcat([frame_cannys, img_rois])

    # 비디오를 저장할때는 반드시 COLOR로 바꿔주어야 함
    # Gray나 binary는 1채널이므로 저장이 안됨
    writer.write(result_cont1)
    writer1.write(cont4)
    ...
    img_frame2 = cv2.pyrDown(cont4)
    img_frame3 = cv2.pyrDown(img_frame2)
    cv2.imshow('Video', img_frame3)

    result_cont_pydown = cv2.pyrDown(result_cont1)
    cv2.imshow('result_cont_pydown', result_cont_pydown)
